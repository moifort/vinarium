import { randomWineId } from '~/domain/wine/primitives'
import * as _repository from '~/domain/wine/repository'
import type { Wine, WineColor, WineId, WineName } from '~/domain/wine/types'
import { traced, tracedModule } from '~/system/sentry/tracing'

const repository = tracedModule('wine', 'db', _repository)

export namespace WineCommand {
  export const add = traced(
    'WineCommand.add',
    'domain.command',
    async (name: WineName, color: WineColor, data: Partial<Wine>) => {
      return await repository.save({
        ...data,
        id: randomWineId(),
        name,
        color,
        createdAt: new Date(),
        updatedAt: new Date(),
      })
    },
  )

  export const update = traced(
    'WineCommand.update',
    'domain.command',
    async (id: WineId, data: Partial<Wine>) => {
      const existing = await repository.findBy(id)
      if (!existing) return 'not-found' as const
      return await repository.save({
        ...existing,
        ...data,
        updatedAt: new Date(),
      })
    },
  )

  export const remove = traced('WineCommand.remove', 'domain.command', async (id: WineId) => {
    const existing = await repository.findBy(id)
    if (!existing) return 'not-found' as const
    await repository.remove(id)
    return
  })
}
