...
export namespace CellarGrid {
  export const isPositionTaken = (entries: CellarEntry[], row: number, col: number) =>
  ...
  export const suggest = async (wineId: WineId, entries: CellarEntry[]) => {
    ...
    const occupied = entries.map((entry) => `${entry.row},${entry.col}`)
  ...
  export const get = async (entries: CellarEntry[]) => {
    const grid: GridCell[][] = Array.from({ length: ROWS }, (_, rowIdx) =>
      Array.from({ length: COLS }, (_, colIdx) => ({
        position: `${CellarRow.toLabel(CellarRow(rowIdx))}${CellarCol.toLabel(CellarCol(colIdx))}`,
      })),
    )
    await Promise.all(
      entries
        .filter((entry) => entry.row < ROWS && entry.col < COLS)
        .map(async (entry) => {
          const wine = await Wines.getById(entry.wineId)
          if (wine !== 'not-found') {
            grid[entry.row][entry.col].wine = wine
          }
        }),
    )
    return grid
