...
- **GraphQL** (Apollo Server + Pothos, single endpoint `POST /api/graphql`): nested `WineType` fields must never scan a collection or read one doc per parent row (N+1). List paths pre-attach satellites in batch (`assemble` in `wine/infrastructure/graphql/queries.ts`, via `getManyByWineIds`); the resolver fallback (detail path) does a single keyed read (`getByWineId`, doc `${userId}_${wineId}`). Read budgets are asserted in tests via `fake.docReads`/`fake.queryReads`
